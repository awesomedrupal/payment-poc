...
const app = express();
app.use(tracingMiddleware(service, log));
...
app.get("/time", (_req, res) =>
  res.json({ server_time_unix: Math.floor(Date.now() / 1000) }),
);
...
app.post("/payment_intents", hmacAuth, async (req, res) => {
  ...
  const traceId = req.trace.trace_id;

  if (!idem) return res.status(400).json({ error: "missing_idempotency_key" });
  if (!amount_cents || !currency)
    return res.status(400).json({ error: "missing_amount_or_currency" });
  ...
  try {
    const result = await withTx(async (client) => {
      ...
      if (existing.rows.length) {
        return {
          payment_intent_id: existing.rows[0].id,
      ...
      await client.query(
        ...
        [
          uuid(),
          paymentId,
          JSON.stringify({ amount_cents, currency, order_ref }),
        ],
      );
      ...
      await client.query(
        ...
        [
          uuid(),
          paymentId,
          JSON.stringify({ payment_id: paymentId, merchant_id: merchantId }),
          `${paymentId}:PAYMENT_INTENT_CREATED`,
        ],
      );
      ...
      metrics.paymentTransitionsTotal.inc({
        service,
        from_status: "NONE",
        to_status: "REQUIRES_PAYMENT_METHOD",
        source: "api",
      });
...
app.post("/payment_intents/:id/confirm", async (req, res) => {
  ...
  const traceId = req.trace.trace_id;
  if (!client_secret || !payment_method_token)
    return res.status(400).json({ error: "missing_client_secret_or_token" });
  ...
  try {
    const out = await withTx(async (client) => {
      ...
      const pi = rows[0];
      if (new Date(pi.client_secret_expires_at).getTime() < Date.now())
        return { status: 401, body: { error: "client_secret_expired" } };
      if (sha256Hex(client_secret) !== pi.client_secret_hash)
        return { status: 401, body: { error: "invalid_client_secret" } };

      if (["SUCCEEDED", "FAILED", "CANCELED"].includes(pi.status)) {
        return {
          status: 200,
          body: { payment_intent_id: id, status: pi.status, note: "terminal" },
        };
      }
      ...
      await client.query(
        ...
        [
          uuid(),
          id,
          JSON.stringify({ payment_id: id, payment_method_token }),
          `${id}:PAYMENT_CONFIRM_REQUESTED:${Date.now()}`,
        ],
      );
      metrics.paymentTransitionsTotal.inc({
        service,
      ...
      return {
        status: 200,
        body: { payment_intent_id: id, status: "PROCESSING" },
      };
    });
    log("info", "payment_confirm_requested", {
      trace_id: traceId,
      payment_id: id,
      status: "PROCESSING",
    });
    ...
    log("error", "confirm_failed", { trace_id: traceId, error: e.message });
...
app.post("/webhooks/gateway", async (req, res) => {
  ...
  if (sig !== expected) {
    metrics.webhookEventsTotal.inc({ service, status: "bad_signature" });
    return res.status(401).json({ error: "bad_webhook_signature" });
  }
  ...
  if (!gateway_event_id || !payment_id || !status)
  ...
  try {
    ...
    await withTx(async (client) => {
      ...
      await client.query(
        ...
        [
          uuid(),
          payment_id,
          nextStatus === "SUCCEEDED" ? "GATEWAY_SUCCEEDED" : "GATEWAY_FAILED",
          current,
          nextStatus,
          JSON.stringify({
            gateway_event_id,
      ...
      await client.query(
        ...
        [
          uuid(),
          payment_id,
          nextStatus === "SUCCEEDED" ? "PAYMENT_SUCCEEDED" : "PAYMENT_FAILED",
    ...
    if (duplicate) {
      metrics.webhookDuplicatesTotal.inc({ service });
...
app.listen(port, () => log("info", "api_started", { port }));

setInterval(() => {
  query(`DELETE FROM used_nonces WHERE expires_at < now()`).catch(() => {});
}, 30_000);
